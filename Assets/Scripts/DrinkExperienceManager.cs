...
public class DrinkExperienceManager : Singleton<DrinkExperienceManager>
{
    [Tooltip("Gameobject of drink platform to create for drink selection.")]
    public GameObject drinkPlatform;

    public void OnShowDrinks()
    {
        List<GameObject> floors = SurfaceMeshesToPlanes.Instance.GetActivePlanes(PlaneTypes.Floor);
        if (floors.Count == 0)
        {
            Debug.Log("No floor found yet.");
        } else
        {
